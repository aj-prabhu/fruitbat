...
// The dev-Space capture (S1-00b, 2026-09-24) showed the browser requests only the asyncify build.
const wanted = (await readdir(src)).filter((f) => /^ort-wasm-simd-threaded\.asyncify\.(mjs|wasm)$/.test(f));
